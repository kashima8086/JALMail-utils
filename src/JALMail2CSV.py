...
# JALカードボーナス適用
# =========================
def apply_card_bonus(rows: List[UnifiedRow], enable_first_flight_bonus: bool = False) -> None:
    ...
    for row in rows:
        ...
        # 年初回搭乗ボーナス
        if enable_first_flight_bonus and not yearly_first_applied[year]:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="JAL国内線購入通知メールをCSVに変換します。"
    )
    parser.add_argument(
        "--first-flight-bonus",
        action="store_true",
        help="JALカードの年初回搭乗ボーナスを有効化します。",
    )
    ...
    apply_card_bonus(all_rows, enable_first_flight_bonus=args.first_flight_bonus)
