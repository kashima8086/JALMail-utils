...
class JALMailConfig:
    ...
    mail_subject_regex: str = MAIL_SUBJECT_REGEX
    card_bonus_type: str = CARD_BONUS_TYPE
    tour_premium: int = TOUR_PREMIUM


class JALMailEngine:
    """
    JAL国内線購入通知メールの取得・解析エンジン。

    既存の JALMail2CSV1.0.py 相当の解析ロジックをクラス化したものです。
    本体プログラムからは、このクラスの get_rows_from_imap() または
    get_rows_from_texts() を呼び出してください。
    """

    def __init__(self, config: Optional[JALMailConfig] = None):
        self.config = config or JALMailConfig()
        self._apply_config()

    def _apply_config(self) -> None:
        """既存関数群が参照するモジュールグローバルへ設定を反映する。"""
        global IMAP_HOST, IMAP_PORT, IMAP_USER, IMAP_PASSWORD, IMAP_FOLDER
        global MAIL_SUBJECT_REGEX, CARD_BONUS_TYPE, TOUR_PREMIUM
        ...
        MAIL_SUBJECT_REGEX = self.config.mail_subject_regex
        CARD_BONUS_TYPE = self.config.card_bonus_type
        TOUR_PREMIUM = self.config.tour_premium

    def fetch_matching_mail_texts(self) -> List[str]:
        """IMAPサーバーから対象メール本文を取得する。"""
        self._apply_config()
        return fetch_matching_mail_texts()

    def parse_mail_text(self, text: str) -> List[UnifiedRow]:
        """JAL購入通知メール本文を解析して UnifiedRow のリストを返す。"""
        self._apply_config()
        return parse_mail_text(text)

    def apply_card_bonus(self, rows: List[UnifiedRow]) -> None:
        """JALカードボーナスを行リストへ適用する。"""
        self._apply_config()
        apply_card_bonus(rows)

    def get_rows_from_imap(self, apply_bonus: bool = True) -> List[UnifiedRow]:
        """IMAPからメールを取得し、解析済み行リストを返す。"""
        rows: List[UnifiedRow] = []
        for mail_text in self.fetch_matching_mail_texts():
            rows.extend(self.parse_mail_text(mail_text))
        if apply_bonus:
            self.apply_card_bonus(rows)
        return rows

    def get_rows_from_texts(self, texts: Iterable[str], apply_bonus: bool = True) -> List[UnifiedRow]:
        """保存済みメール本文テキスト群を解析し、行リストを返す。"""
        rows: List[UnifiedRow] = []
        for text in texts:
            rows.extend(self.parse_mail_text(text))
        if apply_bonus:
            self.apply_card_bonus(rows)
        return rows

    def get_rows_from_files(self, paths: Iterable[str], encoding: str = "utf-8", apply_bonus: bool = True) -> List[UnifiedRow]:
        """保存済みメール本文ファイル群を読み込んで解析する。"""
        texts = []
        for path in paths:
            with builtins.open(path, "r", encoding=encoding) as f:
                texts.append(f.read())
        return self.get_rows_from_texts(texts, apply_bonus=apply_bonus)
...
def open(
    ...
    """
    JALMailEngine インスタンスを生成して返すファクトリ関数。

    使用例:
        import JALMailEngine
        obj = JALMailEngine.open(IMAP_HOST, IMAP_PORT, IMAP_USER, IMAP_PASSWORD, IMAP_FOLDER)
        rows = obj.get_rows_from_imap()
    """
    config = JALMailConfig(
        imap_host=imap_host,
        imap_port=imap_port,
        imap_user=imap_user,
        imap_password=imap_password,
        imap_folder=imap_folder,
        mail_subject_regex=mail_subject_regex,
        card_bonus_type=card_bonus_type,
        tour_premium=tour_premium,
    )
    return JALMailEngine(config)
